...
def maak_een_nieuwe_header(super_dict):
    file1 = open("uber_file_totlocatieTEST.csv","r")
...
def het_vinden_en_toevoegen(super_dict,header,kleine_header):
    file1 = open("new_cor_file_TOETS_MET_THRESHOLD.csv","w")
    file1.write(header+"\n")

    regels_uit_uber =roep_locatie_file("uber_file_totlocatieTEST.csv")
...
def maak_dictionary():
    dict_koppel = {}
    file2 = open("k-nn_Interactions_relation.toets.csv", "r")
